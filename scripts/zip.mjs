 ...
 *
 * The Chrome Web Store wants a zip. The previous `npm run zip` shelled out to
 * the Unix `zip` binary, which does not exist on a stock Windows machine, so
 * packaging silently only worked for some contributors. Node already ships
 * deflate, and a store-format zip is a few dozen lines, so the archive is
 * written here instead: same output on every platform, no dependency.
 *
 * Every entry gets a fixed 1980-01-01 timestamp so that building the same
 * source twice produces a byte-identical archive. That makes the artefact
 * reproducible and lets a reviewer diff two releases.
 */
 ...
 * (the file exists, it has the right size) while producing something the Web
